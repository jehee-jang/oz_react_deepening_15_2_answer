...
return (
  <div className="w-full flex flex-col">
    <div className="w-full h-[60px] bg-stone-200 rounded-sm flex items-center justify-center">
      ...
      <div
        ref={setNodeRef}
        className={`flex flex-col gap-2 p-4 min-h-[200px] rounded-md transition 
          ${isOver ? 'bg-slate-200 ring-2 ring-slate-400 ring-inset' : ''}`}
      >
        <SortableContext items={filteredData.map((it) => it.id)} strategy={verticalListSortingStrategy}>
          {filteredData.map((it) => (
            <SortableItem
              key={it.id}
              id={it.id}
              item={it}
              onClick={() => handleModalOpen(it)}
            />
          ))}

          {filteredData.length === 0 && (
            <div className="flex-1 h-[180px] border-2 border-dashed border-slate-300 rounded-md flex items-center justify-center text-slate-400">
              이 영역으로 항목을 드래그하세요
            </div>
          )}
        </SortableContext>
